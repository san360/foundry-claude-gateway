# Claude models in Microsoft Foundry â€” direct and AI gateway demo

An end-to-end, deployable demonstration of **Anthropic Claude models running in Microsoft Foundry**, consumed two ways:

1. **Direct** â€” Claude Code (and any Anthropic SDK client) calls the Foundry Anthropic endpoint.
2. **Via the AI gateway** â€” the same clients call **Azure API Management**, which fronts Foundry with token governance, per-caller quotas, telemetry and centralized identity.
...
| `accounts/deployments` Ã— 1â€“3 | Claude Haiku / Sonnet / Opus, `GlobalStandard` |
...
| Doc | Contents |
| --- | --- |
| [01 â€” Architecture](docs/01-architecture.md) | Components, request flows, design decisions and why each one was made |
| [02 â€” Deploy](docs/02-deploy.md) | Prerequisites, parameters, deployment, teardown |
| [03 â€” Claude Code direct](docs/03-claude-code-direct.md) | Every environment variable, model pinning, desktop app and CLI |
| [04 â€” Claude Code via the AI gateway](docs/04-claude-code-gateway.md) | Gateway configuration, policy walkthrough, governance features |
| [05 â€” Entra authentication](docs/05-entra-authentication.md) | **The auth matrix**: does Entra work for both paths, and how |
| [06 â€” Demo script](docs/06-demo-script.md) | A 20-minute run-of-show with talk track and expected output |
| [07 â€” Troubleshooting](docs/07-troubleshooting.md) | Error-by-error diagnosis for both paths |
